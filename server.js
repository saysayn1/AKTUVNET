...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        // Allow all common file types including voice messages
        const allowedMimeTypes = [
            'image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml',
            'application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
            'text/plain', 'audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/ogg', 'audio/webm',
            'video/mp4', 'video/webm', 'video/quicktime', 'video/x-msvideo', 'video/avi',
            'application/zip', 'application/x-rar-compressed', 'application/x-7z-compressed'
        ];
        
        const allowedExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.pdf', '.doc', '.docx',
                                   '.txt', '.mp3', '.wav', '.ogg', '.webm', '.mp4', '.mov', '.avi', '.zip', '.rar', '.7z'];
...
// Update game status
app.post('/api/user/game-status', authenticateToken, async (req, res) => {
    try {
        const { gameStatus } = req.body;
        await userDB.updateGameStatus(req.user.id, gameStatus);
        
        // Broadcast to all connected users
        const user = await userDB.findById(req.user.id);
        io.emit('user-game-status-update', {
            userId: req.user.id,
...
app.post('/api/friends/accept', authenticateToken, async (req, res) => {
    try {
        ...
        const userSocket = Array.from(users.values()).find(u => u.id === req.user.id);
        const friendSocket = Array.from(users.values()).find(u => u.id === friendId);
        
        if (userSocket) {
            io.to(userSocket.socketId).emit('friend-list-update');
        }
        if (friendSocket) {
            io.to(friendSocket.socketId).emit('friend-list-update');
        }
...
io.on('connection', async (socket) => {
    ...
    socket.on('typing-start', async (data) => {
        try {
            const { receiverId, channelId, isDM } = data;
            const user = await userDB.findById(socket.userId);
            
            if (isDM && receiverId) {
                // Send typing indicator to DM recipient
    ...
    socket.on('typing-stop', async (data) => {
        try {
            ...
            if (isDM && receiverId) {
                // Send stop typing to DM recipient
                const receiverSocket = Array.from(users.values()).find(u => u.id === receiverId);
                if (receiverSocket) {
                    io.to(receiverSocket.socketId).emit('user-stopped-typing', {
                        userId: socket.userId
                    });
                }
            } else if (channelId) {
    ...
    // User disconnected from call (not ended, just disconnected)
    socket.on('call-participant-disconnected', (data) => {
        const { to, callId } = data;
        if (to) {
            // Notify other participant that user disconnected
            io.to(to).emit('call-participant-left', { 
                from: socket.id,
                callId: callId,
                waitTime: 300000 // 5 minutes in milliseconds
            });
            
            // Set timeout to auto-end call after 5 minutes
            setTimeout(() => {
                io.to(to).emit('call-timeout', { callId: callId });
            }, 300000);
        }
    });
...
// Start server
server.listen(PORT, () => {
    console.log(`🎮 AKTUVNET server running on http://localhost:${PORT}`);
    console.log(`🚀 Open http://localhost:${PORT}/login.html in your browser`);
    console.log(`⚡ Gaming Communication Platform Ready!`);
});